...
export default function PendingDocuments({ isActive, searchQuery }: PendingDocumentsProps) {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      dispatch(getPendings({ data: { data: searchResponse.data, limit: searchResponse.data?.length || 0 } }));

      // Update pagination
      let total = 0;
      if (typeof searchResponse.data === "object" && searchResponse.data !== null) {
        if (searchResponse.data.total && typeof searchResponse.data.total === "number") {
                ...
                <div className="bg-muted/40 px-2 py-1 rounded flex items-center">
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent>{copiedSessionId && copiedSessionIdText === sessionId ? "Copied!" : "Copy"}</TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </div>
              </div>
